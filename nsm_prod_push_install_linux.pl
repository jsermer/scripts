...
print "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
print "Please push the NSM install base to the server from TUNIM01!\n";
print "ssh tunim01\n";
print "rsync -av /opt/install/src/nsm31_xlinux CHANGEME:/tmp/.\n";
print "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n";
...
foreach my $remote_host (@hosts) {
	...
	my $root_cmd1 = "awservices stop; unishutdown all; cp -p /etc/sysctl.conf /etc/sysctl.conf.\$(date +%H%M_%m%d%y).NSM; cp -p /etc/profile /etc/profile.\$(date +%H%M_%m%d%y).NSM; scp -r $ssh_sudo_user\@10.2.15.21:/opt/install/src/nsm31_xlinux /tmp/.; cd /tmp/nsm31_xlinux/; ./setupNSM";
	my $root_cmd2 = "echo -e \"SNMP_COMMUNITY public|0.0.0.0|read # Any host can read with public\nSNMP_COMMUNITY admin|0.0.0.0|write # Any host can write using admin\nSNMP_TRAP 10.2.8.172|162 # traps to NSMCOL01\nSNMP_TRAP 10.2.8.116|162 # traps to NSMCOR\nSNMP_TRAP 10.2.8.171|162 # traps to NSMCOL02\nSNMP_TRAP 10.2.9.7|162 # traps to NSMMON\nSNMP_TRAP 10.2.9.5|162 # traps to NSMCORPP\nSNMP_TRAP 10.8.32.75|162 # traps to chi1wesm9nsm04.corp.transunion.com\nSNMP_TRAP 10.8.32.76|162 # traps to chi1wesm9nsm05.corp.transunion.com\nSNMP_TRAP 10.8.32.78|162 # traps to chi1wesm9nsm07.corp.transunion.com\nSNMP_TRAP 10.8.32.80|162 # traps to chi1wesm9nsm09.corp.transunion.com\nSNMP_TRAP 10.8.32.81|162 # traps to chi1wesm9nsm10.corp.transunion.com\" > /opt/CA/UnicenterNSM/atech/services/config/aws_sadmin/aws_sadmin.cfg";
	my $root_cmd3 = "echo -e \"#PLUGIN awm_qikpipe aws_orb22\nPLUGIN awm_qiksoc 7774\nPLUGIN awm_qiksoc20 7770\nINFORM_REMOTE YES\nHEARTBEAT_FREQUENCY 120\" > /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg; source /etc/profile; awservices start; date | mail -s \"NSM installed on $remote_host\" $ssh_sudo_user\@tuc.com";


#	$ssh_session->exp_internal(1);
#	$ssh_session->max_accum(80);
#	$ssh_session->raw_pty(1);
	$ssh_session->log_file("nsm_prod_push_install_linux.log");
