...
foreach my $remote_host (@hosts) {
	...
	my $root_cmd1 = "awservices stop; unishutdown all; cp -p /etc/profile /etc/profile.\$(date +%H%M_%m%d%y).NSM; mount -o ro 10.2.15.21:/opt/install /mnt; cd /mnt/src/nsm31_aix; ./setupNSM";
	my $root_cmd2 = "cd /root; umount /mnt; echo \"SNMP_COMMUNITY  public|0.0.0.0|read        # Any host can read with public\nSNMP_COMMUNITY  admin|0.0.0.0|write        # Any host can write using admin\nSNMP_TRAP       127.0.0.1|162              # traps to localhost\nSNMP_TRAP       10.2.8.116|162             # traps to NSMCOR\nSNMP_TRAP       10.2.8.171|162             # traps to NSMCOL02\" > /opt/CA/UnicenterNSM/atech/services/config/aws_sadmin/aws_sadmin.cfg";
	my $root_cmd3 = "echo \"#PLUGIN awm_qikpipe aws_orb22\nPLUGIN awm_qiksoc 7774\nPLUGIN awm_qiksoc20 7770\nINFORM_REMOTE YES\nHEARTBEAT_FREQUENCY 120\" > /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg; . /etc/profile; awservices start; date | mail -s \"NSM installed on $remote_host\" $ssh_sudo_user\@tuc.com";


#	$ssh_session->exp_internal(1);
#	$ssh_session->max_accum(80);
#	$ssh_session->raw_pty(1);
	$ssh_session->log_file("nsm_prod_install_aix.log");
