...
foreach my $remote_host (@hosts) {
	...
	my $root_cmd1 = "awservices stop; unishutdown all; cp -p /etc/sysctl.conf /etc/sysctl.conf.\$(date +%H%M_%m%d%y).NSM; cp -p /etc/profile /etc/profile.\$(date +%H%M_%m%d%y).NSM; mount -o ro 10.2.15.21:/opt/install /mnt; cd /mnt/src/nsm31_zlinux; ./setupNSM";
	my $root_cmd2 = "cd /root; umount /mnt; echo -e \"SNMP_COMMUNITY  public|0.0.0.0|read        # Any host can read with public\nSNMP_COMMUNITY  admin|0.0.0.0|write        # Any host can write using admin\nSNMP_TRAP       127.0.0.1|162              # traps to localhost\nSNMP_TRAP       10.2.8.116|162             # traps to NSMCOR\nSNMP_TRAP       10.2.8.171|162             # traps to NSMCOL02\" > /opt/CA/UnicenterNSM/atech/services/config/aws_sadmin/aws_sadmin.cfg";
...
#	$ssh_session->raw_pty(1);
	$ssh_session->log_file("nsm_prod_install_zlinux.log");
	...
	$ssh_session->expect($timeout,
		...
		[ qr/\(\$AGENTWORKS_DIR\) \(default: \/opt\/CA\/NSM\/atech\)/, sub { 
			my $self = shift;
			$self->send("/opt/CA/UnicenterNSM/atech\n");
			exp_continue;
		} ],
		...
		[ qr/Is this correct\? \(y\/n\) \(default: n\)/, sub { 
			my $self = shift;
			$self->send("y\n");
			exp_continue;
		} ],
		[ qr/Is this what you want\? \(y\/n\/q\) \(default: y\)/, sub { 
			my $self = shift;
			$self->send("y\n");
			exp_continue;
		} ],
		...
		[ qr/Please specify the host name for the NSM Event Manager Node:/, sub { 
			my $self = shift;
			$self->send("10.2.8.116\n");
			exp_continue;
		} ],
		...
		[ qr/Please specify the directory:/, sub { 
			my $self = shift;
			$self->send("/var/tmp\n");
			exp_continue;
		} ],
		[ qr/Please specify the installation directory \(Type exit to quit\):/, sub { 
			my $self = shift;
			$self->send("/opt/CA/UnicenterNSM\n");
			exp_continue;
		} ],
		[ qr/Do you want the rc file updated\? \(y\/n\) \(default: y\)/, sub { 
			my $self = shift;
			$self->send("y\n");
			exp_continue;
		} ],
