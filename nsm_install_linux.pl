...
foreach my $remote_host (@hosts) {
	...
	my $root_cmd1 = "awservices stop; unishutdown all; cp -p /etc/sysctl.conf /etc/sysctl.conf.b4NSM; cp -p /etc/profile /etc/profile.b4NSM; mount -o ro tunim01:/opt/install /mnt; cd /mnt/src/nsm31_xlinux; ./setupNSM";
	...
	DONE";
	my $root_cmd3 = "cp /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg.BKP; sed 's/^PLUGIN awm_qikpipe aws_orb22/#PLUGIN awm_qikpipe aws_orb22/' /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg.BKP > /opt/CA/UnicenterNSM/atech/services/config/aws_orb/quick.cfg; source /etc/profile; awservices start; date | mail -s \"NSM installed on $remote_host\" $ssh_sudo_user\@tuc.com";
