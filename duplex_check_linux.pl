...
foreach my $remote_host (@hosts) {
	...
	my $root_cmd1 = "wget http://10.2.15.21/js/bin/duplex_check.sh -O /usr/local/bin/duplex_check.sh";
	my $root_cmd2 = "chmod 544 /usr/local/bin/duplex_check.sh";
	my $root_cmd3 = "/usr/local/bin/duplex_check.sh";
